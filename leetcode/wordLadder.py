...
class Solution(object):
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        graph = SimpleGraph(beginWord)
        print("---Constructed graph in: %s seconds ---" % (time.time() - start_time))

        return dijkstraSearch(graph, beginWord, endWord, wordList)
        
        

# priority queue wrapper class around heapq
class PriorityQueue:
    def __init__(self):
        self.frontier = []
    def empty(self):
...
'''
do a bfs on a graph generated "on the fly" by computing string permutations with
a hamming distance of 1 at each step
store strings in the queue with priority = distance from original word
'''
def bfs(beginWord, endWord, wordList):
    # all words the same length
    length = len(beginWord)

    # keep track of words we visited
    ...
    while not q.empty():
        ...
        # visit all neighbors by generating all possible string permutations
        # check if generated string exists in the dictionary
        neighbors = [curr_word[:i] + ch + curr_word[i+1:] for ch in LETTERS for i in range(length)]
...
class SimpleGraph:
    ...
    # we'll compute the neighbors on the fly at each word 
    def neighbors(self, curr_word, wordList):
        #neighs =  [(curr_word[:i] + ch + curr_word[i+1:] for ch in LETTERS if (curr_word[:i] + ch + curr_word[i+1:])) in wordList for i in range(self.length) ]
        neighs = []
        for i in range(self.length):
            for ch in LETTERS:
                next = curr_word[:i] + ch + curr_word[i+1:]
                if next in wordList:
                    neighs.append(next)
        return neighs
        
# construct a SimpleGraph. Computationally expensive to build for large dictionaries
...
def dijkstraSearch(graph, start, goal, wordList):
    ...
    while not frontier.empty():
        curr_word = frontier.get()
        
        if curr_word == goal:
            break
        
        # find neighbors and put into queue if they are in the wordlist
        for i in range(length):
            for ch in LETTERS:
                next = curr_word[:i] + ch + curr_word[i+1:]
                new_cost = cost_so_far[curr_word] + 1
                if next in wordList and (next not in cost_so_far or new_cost < cost_so_far[next]):
                    cost_so_far[next] = new_cost
                    priority = new_cost
                    frontier.put(next, priority)
      
    # return the cost to get goal, if there was a way to get there
