...
of edges (26510), and it takes considerable time to build this graph. Since we don't visit many of the nodes, and
since we can generate the graph at every itteration because there is a direct mapping from strings to the graph structure,
...
def constructGraph(beginWord, endWord, wordList):
    ...
    edges = 0
    for word in graph.edges:
        edges += len(graph.edges[word])

    print "number of edges: {0}".format(edges)
